...
import { generatePoskoReportDocx } from "@/lib/posko-report-export";
...
function buildFilename(report) {
  ...
  return `laporan-${typePart}-${datePart}-${report.reportCode}.docx`;
}
...
export async function GET(_request, context) {
  try {
    ...
    const bytes = await generatePoskoReportDocx(report);
    ...
    return Response.json({ message: "Gagal generate DOCX laporan" }, { status: 500 });
  }
}
