...
import { MOTOR_UNIT_STATUS } from "@/lib/constants";
import { db } from "@/lib/db";
import { createMotorUnit, updateMotorUnitStatus } from "./actions";

function statusBadgeClass(status) {
  if (status === MOTOR_UNIT_STATUS.STANDBY) {
    return "bg-emerald-100 text-emerald-700";
  }
  if (status === MOTOR_UNIT_STATUS.BERTUGAS) {
    return "bg-amber-100 text-amber-700";
  }
  return "bg-slate-100 text-slate-700";
}

export default async function MotorUnitPage({ searchParams }) {
    ...
    <main className="min-h-screen bg-slate-50 p-6 md:p-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-2xl font-bold text-slate-900">Armada Motor</h1>
          <p className="mt-2 text-sm text-slate-600">
            Kelola armada motor operasional untuk pergerakan cepat petugas lapangan.
          </p>
        </header>
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr,2fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Tambah Armada Motor</h2>
            <form action={createMotorUnit} className="mt-4 space-y-3">
              ...
              <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">
                ...
                <input
                  required
                  name="vehicleName"
                  placeholder="Motor Operasional 1"
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900"
                />
              </label>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Daftar Armada Motor</h2>
            <p className="mt-1 text-xs text-slate-500">Status armada dapat diubah sesuai kondisi lapangan.</p>
                ...
                <tbody>
                  ...
                  {units.map((unit) => (
                    <tr key={unit.id} className="border-b border-slate-100 last:border-b-0">
                      ...
                      <td className="px-3 py-3">
                        <form action={updateMotorUnitStatus} className="space-y-2">
                          ...
                          >
                            {Object.values(MOTOR_UNIT_STATUS).map((status) => (
